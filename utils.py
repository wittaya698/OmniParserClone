...
def get_caption_model_processor(
    ...
    if model_name == "blip2":
        ...
        if device == "cpu":
            ...
            model = Blip2ForConditionalGeneration.from_pretrained(
                model_name_or_path, device_map=None, torch_dtype=torch.bfloat16
            )
        else:
            model = Blip2ForConditionalGeneration.from_pretrained(
    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        if "florence" in model.config.name_or_path:
            generated_ids = model.generate(
                ...
                max_new_tokens=10,  # to reduce memory usage
                num_beams=1,  # to reduce memory usage
                do_sample=False,
            )
        else:
            generated_ids = model.generate(
...
def get_som_labeled_img(
    ...
    if use_local_semantics:
        ...
        if "phi3_v" in caption_model.config.model_type:
            raise NotImplementedError("phi3_v model type need implemented")
        else:
...
def get_xywh_yolo(input):
    raise NotImplementedError("get_xywh_yolo() need implementation")
